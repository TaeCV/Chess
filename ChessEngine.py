...
class GameState():
    ...
    def MakeMove(self, move, PawmPromotion=''):
        ...
        # En passant
        if move.IsEnPassant:
            self.board[move.PossibleEnPassant[0]
        ...
        if move.CastleKingSide:
            if self.WhiteToMove:
                # Replace Rook Position to be blank
                self.board[move.startRow][move.startCol + 3] = "--"
        ...
        if move.CastleQueenSide:
            if self.WhiteToMove:
                # Replace Rook Position to be blank
                self.board[move.startRow][move.startCol - 4] = "--"
                self.board[move.startRow][move.startCol - 1] = "wR"
            else:
                # Replace Rook Position to be blank
                self.board[move.startRow][move.startCol - 4] = "--"
    ...
    def UndoMove(self):
        if len(self.MoveLog) != 0:
            ...
            if move.IsEnPassant:
                self.board[move.PossibleEnPassant[0]
                           ][move.PossibleEnPassant[1]] = move.PossibleEnPassant[2]
            ...
            if len(self.CastleRightsLog) != 0:
                ...
                # set the castle right to the last move
                self.CurrentCastlingRight = CastleSide(
                    TempCastle.wK, TempCastle.wQ, TempCastle.bK, TempCastle.bQ)

            # Returning Rook after we undo castle move
            if move.CastleKingSide:
                if self.WhiteToMove:
                    # Replace Rook Position to be blank
                    self.board[move.startRow][move.startCol + 3] = "wR"
                    self.board[move.startRow][move.startCol + 1] = "--"
                else:
                    # Replace Rook Position to be blank
                    self.board[move.startRow][move.startCol + 3] = "bR"
                    self.board[move.startRow][move.startCol + 1] = "--"

            if move.CastleQueenSide:
                if self.WhiteToMove:
                    # Replace Rook Position to be blank
                    self.board[move.startRow][move.startCol - 4] = "wR"
                    self.board[move.startRow][move.startCol - 1] = "--"
                else:
                    # Replace Rook Position to be blank
                    self.board[move.startRow][move.startCol - 4] = "bR"
                    self.board[move.startRow][move.startCol - 1] = "--"
        ...
        if self.InCheck:
            if len(self.checks) == 1:  # King needs to move or piece move to block
                ...
                if PieceCheck[1] == "N":  # If the checker is knight, we should capture it
                    ...
                    for i in range(1, 8):
                        # (check[2], check[3]) is the direction of checking
                        ValidSquare = (
                            KingRow + check[2]*i, KingCol + check[3]*i)
                        ValidSquares.append(ValidSquare)
                        # We get all the squares until we capture that's piece
                        if ValidSquare == (CheckRow, CheckCol):
                            break
                # Get rid of the moves that don't block check or move king
                for i in range(len(moves) - 1, -1, -1):
                    # If it's not a king, it should block or capture the piece
        ...
        for row in range(len(self.board)):
            for col in range(len(self.board[row])):
                ...
                if (self.WhiteToMove and color == 'w') or (color == 'b' and not self.WhiteToMove):
                    ...
                    # Call the function on each piece
    ...
    def GetPawnMoves(self, row, col, moves):
        ...
        if col > 0 and self.board[row + CaptureLeft[0]][col + CaptureLeft[1]][0] == EnermyColor:
            if not PiecePinned or PinDirection == CaptureLeft:
                moves.append(
                    Move((row, col), (row + CaptureLeft[0], col + CaptureLeft[1]), self.board))
        ...
        if col < len(self.board[row])-1 and self.board[row + CaptureRight[0]][col + CaptureRight[1]][0] == EnermyColor:
            if not PiecePinned or PinDirection == CaptureRight:
                moves.append(
                    Move((row, col), (row + CaptureRight[0], col + CaptureRight[1]), self.board))

        # Check for normal move
        if self.board[row + MoveDirection[0]][col] == "--":  # Check the obstacle
            if not PiecePinned or PinDirection == MoveDirection:
                # Check the double move obstacle
                if row == StartRow and self.board[row + MoveDirection[0]*2][col] == "--":
                    moves.append(
                        Move((row, col), (row + MoveDirection[0]*2, col), self.board))
                moves.append(
                    Move((row, col), (row + MoveDirection[0], col), self.board))
        ...
        if len(self.MoveLog) != 0:  # En passant
            ...
            if PrevPiece == EnermyPawn and abs(PrevMove.startRow-PrevMove.endRow) == 2 and abs(PrevMove.startCol-col) == 1 and row == StartRow + MoveDirection[0]*3 and not PiecePinned:
                EnPassantMove = Move(
                    (row, col), (PrevMove.startRow - MoveDirection[0], PrevMove.startCol), self.board)
                EnPassantMove.IsEnPassant = True
                EnPassantMove.PossibleEnPassant = (
                    PrevMove.endRow, PrevMove.endCol, EnermyPawn)
...
class Move():
    ...
    def __init__(self, start, end, board):
        ...
        # The coordinate for possible en passant move and the piece captured
