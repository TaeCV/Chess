...
def LoadImages():
    ...
    for piece in pieces:
        Images[piece] = pg.transform.scale(pg.image.load(os.path.join(image_path, piece +".png")), (SQ_size,SQ_size))    #Note: we can access an image by saying Images[type]
...
def main():
    ...
    while running:
                    ...
                    if len(playerClicks) == 2: # time to move the piece
                        ...
                        for i in range (len(ValidMoves)):
                            if move == ValidMoves[i]:
                                ...
                                playerClicks = [] # clear player click
                        if not moveMade:
                            playerClicks = [sqSelected] # To save the last click if we miss click
            elif e.type == pg.KEYDOWN:
                if e.key == pg.K_z and len(GameState.MoveLog) != 0: # undo the move when we press 'z'                    
                    AnimateTheMove(screen, GameState.MoveLog[-1], GameState.board, clock, animateMade)
                    GameState.UndoMove()
                    moveMade = True
        ...
        if moveMade: # to reset the valid move after we moved
            if animateMade: AnimateTheMove(screen, GameState.MoveLog[-1], GameState.board, clock, animateMade)
            ValidMoves = GameState.GetValidMoves()
            moveMade = False
            animateMade = False
...
def ChoosePawnPromotion(screen, side):
    ...
    while running:
        ...
        pg.draw.line(screen, SpecialColor, (Width*3//8-10, Width//2), (Width*5//8+10, Width//2), 2)
        pg.draw.line(screen, SpecialColor, (Width*3//8-10, Width*3//8-10), (Width*5//8+10, Width*3//8-10), 2)
        pg.draw.line(screen, SpecialColor, (Width*3//8-10, Width*5//8+10), (Width*5//8+10, Width*5//8+10), 2)
        pg.draw.line(screen, SpecialColor, (Width//2, Width*3//8-10), (Width//2, Width*5//8+10), 2)
        pg.draw.line(screen, SpecialColor, (Width*3//8-10, Width*3//8-10), (Width*3//8-10, Width*5//8+10), 2)
        pg.draw.line(screen, SpecialColor, (Width*5//8+10, Width*3//8-10), (Width*5//8+10, Width*5//8+10), 2)
...
def drawGameState(screen, GameState, ValidMoves, sqSelected):
    ...
    highlightSquares(screen, GameState, ValidMoves, sqSelected) # Highlight the squares
...
def highlightSquares(screen, GameState, ValidMoves, sqSelected):
    ...
    CheckBox.set_alpha(150) # Tranparency value -> 0 == transparent , 255 = solid
    CheckBox.fill(pg.Color("red"))
    if GameState.InCheck:
        if GameState.WhiteToMove:
            screen.blit(CheckBox, (GameState.WhiteKingLocation[1]*SQ_size, GameState.WhiteKingLocation[0]*SQ_size))
        else:            
            screen.blit(CheckBox, (GameState.BlackKingLocation[1]*SQ_size, GameState.BlackKingLocation[0]*SQ_size))

    if sqSelected != ():
        row , col = sqSelected
        if GameState.board[row][col][0] == ("w" if GameState.WhiteToMove else "b"): # The selected squares is the correct turn
            # Highlight selected square
            s = pg.Surface((SQ_size, SQ_size))
            s.set_alpha(100) # Tranparency value -> 0 == transparent , 255 = solid
...
def drawBoard(screen):
    ...
    current_path = os.path.dirname(__file__)
    image_path = os.path.join(current_path, 'img')
    WhiteWooden = pg.transform.scale(pg.image.load(os.path.join(image_path, "WhiteWooden.png")).convert_alpha(), (SQ_size,SQ_size))
    BrownWooden = pg.transform.scale(pg.image.load(os.path.join(image_path, "BrownWooden.png")).convert_alpha(), (SQ_size,SQ_size))
    Colors = [WhiteWooden, BrownWooden] 
    for row in range (Dimention):
        for col in range (Dimention):
            color = Colors[(row+col) %2]
            screen.blit(color, pg.Rect(col*SQ_size, row*SQ_size, SQ_size, SQ_size)) 

# Draw the wooden board.
def drawWoodenBoard(screen):  
    current_path = os.path.dirname(__file__)
    image_path = os.path.join(current_path, 'img')
    Background = pg.transform.scale(pg.image.load(os.path.join(image_path, "board.png")).convert_alpha(), (Width, Width))
...
def AnimateTheMove(screen, move, board, clock, animateMade):
    ...
    for frame in range (FramesCount + 1):
        if animateMade:
            row, col = (move.startRow + dR*frame/FramesCount, move.startCol + dC*frame/FramesCount)
            color = Colors[(move.endRow + move.endCol) % 2]
            EndSquare = pg.Rect(move.endCol*SQ_size, move.endRow*SQ_size, SQ_size, SQ_size)
        else:
            row, col = (move.endRow - dR*frame/FramesCount, move.endCol - dC*frame/FramesCount)
            color = Colors[(move.startRow + move.startCol) % 2]
            EndSquare = pg.Rect(move.startCol*SQ_size, move.startRow*SQ_size, SQ_size, SQ_size)
            color2 = Colors[(move.endRow + move.endCol) % 2]
            EndSquare2 = pg.Rect(move.endCol*SQ_size, move.endRow*SQ_size, SQ_size, SQ_size)
        drawBoard(screen)
        ...
        if animateMade:
            ...
            screen.blit(Images[move.pieceMoved], pg.Rect(int(col*SQ_size), int(row*SQ_size), SQ_size, SQ_size))
        else:
